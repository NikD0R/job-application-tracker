...
function DroppableColumn({
  ...
  const sortedJobs =
    column.jobApplications?.sort((item1, item2) => item1.order - item2.order) ||
    [];
  ...
  return (
    <Card className="min-w-75 shrink-0 shadow-md p-0">
      ...
      <CardContent
        ref={setNodeRef}
        className={`space-y-2 pt-4 bg-muted/30 min-h-100 rounded-b-lg ${isOver ? "ring-2 ring-blue-500" : ""}`}
      >
        <SortableContext
...
export default function KanbanBoard({ board }: KanbanBoardProps) {
  ...
  const { moveJob, error, filteredColumns, filters, setFilters } =
    useBoard(board);

  const sortedColumns =
    filteredColumns?.sort((item1, item2) => item1.order - item2.order) || [];
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    // Check if dropped a column or another job
