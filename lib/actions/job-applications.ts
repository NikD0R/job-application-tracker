...
import { Board, Column, JobApplication, StatusHistory } from "../models";
...
export async function updateJobApplication(
  ...
  if (isMovingToDifferentColumn) {
    ...
    const [fromCol, toCol] = await Promise.all([
      Column.findById(currentColumnId).select("name").lean(),
      Column.findById(newColumnId).select("name").lean(),
    ]);
    ...
    try {
      ...
      if (user?.email) {
        await sendStatusChangeEmail({
          ...
          fromColumn: fromCol?.name ?? "Previous",
          toColumn: toCol?.name ?? "New",
        });
      }
    } catch (emailErr) {
...
export async function getStatusHistory(jobApplicationId: string) {
  const session = await getSession();
  ...
  const history = await StatusHistory.find({ jobApplicationId })
    .sort({ changedAt: -1 })
    .lean();

  return { data: JSON.parse(JSON.stringify(history)) };
}
